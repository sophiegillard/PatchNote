...
export const Home = () =>{
    ...
    const {
        ...
        fetchNextPage,
    } = useArticleLoadMoreQuery(activeCategorieIdFilter, activeModuleIdFilter, startDatePublication, endDatePublication, search, userId);

    const lng = i18n.language.toUpperCase(); // log the current language


    const articles = data && data.pages
        .map(page => page.articles.data)
        .flat()
        .filter((article) => ((article[`titre${lng}`] !== null && article[`titre${lng}`] !== undefined && article[`titre${lng}`] !== "") && (article[`description${lng}`] !== `<p><br></p>` && article[`description${lng}`] !== undefined && article[`description${lng}`] !== "")));
    ...
    <SidebarWithHeader>
        <Container m="0" px="5" maxW="none"
            ...
            {(data && ((data.pageParams.length * 10 ) < data.pages[0].articles.totalCount) && (articles.length !== 0) && (articles.length >= 10)) && (
