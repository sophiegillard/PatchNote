...
export const Mailbox = () => {
    ...
    const filteredMessagesById = data && data.pages
        .map(page => page.messages.data)
        .flat()
        .filter(message => message.id === activeEmailId);

    const filteredMessages = data && data.pages
        .map(page => page.messages.data)
        .flat();
    ...
    return(
        ...
        <SidebarWithHeader>
            <Stack minW="100%" gap="4">
                <Flex bgColor="white" borderRadius="md" maxH="83vh" minH="85vh">

                    <Flex direction="column" borderRight="solid 2px" borderColor="gray.300" maxW={{ base: "100%", lg: "30%" }} width={{ base: "100%", lg: "40%" }}>
                        <Stack width="100%" shadow="md" p={3} allowtoggle="true">
                                <HStack justifyContent="space-between" alignItems="start">
                                    <Stack>
                                        <Heading pb={0}>{t('pages.mailbox.mailbox')}</Heading>
                                        <Text pb={4} >{data.pages[0].messages.totalCount} {t('pages.mailbox.messages')} · {data.pages[0].messages.unreadCount} {t('pages.mailbox.unread')}</Text>
                                    </Stack>
                            ...
                            {isFilterPanelOpen &&(
                                <Stack pb={4} >
                                    ...
                                    <HStack>
                                        <Select
                                            options={[ { value: "", label: "Tout" }  ,...listAllCategoryMessage.map(category => ({ value: category.id, label: category.name }))]}
                                        ...
                                        <Select
                                            options={messageStates && messageStates.map(state => ({ id: state.id, value: state.value, label: t(`pages.mailbox.${state.value}`)}))}
                    ...
                    {data && filteredMessagesById.length > 0 && filteredMessagesById.map(message =>
                        (<MailContent
                            id={message.id}
                            ecole={message.ecole}
                    ...
                    <Show below='lg'>
                        {data && filteredMessagesById.map(message =>
                            (<MailContentMobile
                                    isOpen={isOpen}
                                    onClose={onClose}
                                    id={message.id}
