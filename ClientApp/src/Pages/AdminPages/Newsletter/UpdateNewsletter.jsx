...
import { ButtonGroup, Container, Flex, FormControl, FormLabel, Input, Spinner, Stack, Switch, useDisclosure } from "@chakra-ui/react";
...
export const UpdateNewsletter = () => {
    ...
    const { id } = useParams();
    const {
        data: individualNewsletter,
        isLoading,
        error,
        refetch: refetchOneNewsletter,
    } = useQuery(["individualNewsletter", id], () => getNewsletterById(id));
    ...
    const allArticles = [
        {
            id: 0,
            listArticles: individualNewsletter && individualNewsletter.newsletter.articles,
            label: t("modales.newsletter.newsletter_articles_included"),
        },
        {
            id: 1,
            listArticles: individualNewsletter && individualNewsletter.latestArticles,
            label: t("modales.newsletter.newsletter_articles_latest"),
        },
    ...
    const {
        ...
        isError,
    } = useMutation(updateNewsletterById, {
        onSuccess: () => {
            successToast();
        },
        onError: (error) => {
    ...
    return (
        <>
            <SidebarWithHeader>
                ...
                <BackAndTitle pageTitle={`${t("pages.newsletter.newsletter_update")} ${thisNewsletter.id}`} backNavigation={"/newsletter"} />

                <Stack bgColor="white" borderBottomRadius="lg" p={4}>
                    <form onSubmit={handleSubmit(onSubmit)}>
                        <MyModalBody
                            ...
                            message={
                                "Attention la sélection d'articles entraine la suppression du contenu. Souhaitez-vous enregistrer vos modification?"
                            }
                        />

                        <LanguageTabList>
                            <UpdatedTabPanel isNewsLetter={true} errors={errors} register={register} languageCode={"FR"} />
                            <UpdatedTabPanel isNewsLetter={true} errors={errors} register={register} languageCode={"EN"} />
                            <UpdatedTabPanel isNewsLetter={true} errors={errors} register={register} languageCode={"NL"} />
                        </LanguageTabList>
                        ...
                        <Flex alignItems={"baseline"} flexDirection={{ base: "column", lg: "row" }} gap={5}>
                            ...
                            <FormControl id="ArticlesIncluded" maxW="fit-content">
                                ...
                                {errors.ArticlesIncluded && <span>{t("main.forms.error_message_field_required")}</span>}
                            </FormControl>
