...
export const NewsletterHandler = () => {
    ...
    const { mutate: deleteNewsletter } = useMutation(deleteNewsletterById, {
        onSuccess: () => {
            queryClient.invalidateQueries("newsletters");
        },
        onError: (error) => {
        ...
        <SidebarWithHeader>
            ...
            <Container minW="100%" p="0" bgColor={"white"}>
                ...
                <Table colorscheme="SuppBlue" pe={0} bgColor={"white"}>
                    <Show above={"xl"}>
                        <TableHeader
                            headerTitles={tableHeadContentNewsletters}
                                ...
                                >
                                    <Show below="xl">
                                        <Flex flexDirection="column">
                                            <Td fontWeight="bold" borderTopRadius={{ base: "", lg: "xl" }}>
                                                <HStack justifyContent="space-between">
                                                    <Flex flexDirection="column" gap="2">
                                                        <Text textTransform="uppercase" letterSpacing="0.1rem">
                                                            {newsletter.id} | {newsletter.titreFR}
                                                        </Text>
                                                        <Text fontWeight="light">Date de publication : {newsletter.datePublication}</Text>
                                                        <Text fontWeight="light">Nbr d'articles : {newsletter.datePublication}</Text>
                                                    </Flex>
